...
Flow
  ...
  4. The loader finds the memory address of MessageBoxW inside USER32.dll EAT.
...
- Process — block a process from starting by setting STATUS_ACCESS_DENIED
- Object — strip privileges from handles (e.g. remove PROCESS_VM_READ so Mimikatz can't read LSASS memory)
- Registry — block creation of registry keys used for privilege escalation or UAC bypasses

Same principle across all three: find where the kernel stores the callback, and either remove it, disable it, or skip over it.

## ETW-TI

ETW-TI is a telemetry source that EDRs consume. These products subscribe to ETW-TI events as one of their data sources alongside other type of telemetry. 

If you were to make your own tool, you would want to use `JonMon` or `MentalTi` to patch your application's kernel memory, to make the kernel think that your process is running at 

1. PS_PROTECTED_ANTIMALWARE_LIGHT 
2. SERVICE_LAUNCH_PROTECTED_ANTIMALWARE_LIGHT

so that they can consume ETW-TI events. However, you still need `test signing` to be enabled on the machine. 

![How ETW events work](/assets/img/ETWevents.png)

The global variable `EtwpDebuggerData` is in the kernel. So the bucket is the one holding the provider. 

![finding a specific Provider](/assets/img/findingProvider.png)

Basically a provider has a specific GUID. Calculate manually which bucket the windows defender provider is from. 

![disabling a Provider](/assets/img/disablingProvider.png)
